...
def connect_url(url,df,idx):
    ...
    try:
        ...
        elems = soup.find_all("a",href=re.compile('^.*https://tabelog.com/chiba'))
        if not elems:
            print("Nothing")
        else:
            for elem in elems:
        ...
        # 後始末
        pass
df = pd.read_csv('c:/shop_list_utf8.csv', header=None)
#ゼロ埋めする
df = df.assign(url=0)
df = df.assign(elm1=0)
df = df.assign(elm2=0)
...
for idx,row in df.iterrows():
    urlName = "https://www.google.com/search?q=" + re.sub("-","",row[4]) + "+" + row[5] + "+tabelog"
